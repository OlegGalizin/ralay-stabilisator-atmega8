...
{
  LCD_PORT &= ~LCD_RST_PIN; /* RESET to 0 */
  if (Contrast == 0)
    Contrast = 60;
  if (Contrast > 90)
    Contrast = 90;
  ...
  LCD_PORT |= LCD_RST_PIN; /* RESET to 1 */
...
void LcdSend ( uint8_t Data, LcdCmdData cd )
{
  ...
  LCD_PORT &= ~LCD_CE_PIN; /* CE pin to LOW */
  if ( cd != LCD_CMD ) /* If data */
  {
    LCD_PORT |= LCD_DC_PIN; /* DC pin to hight */
  }
  else
  {
    LCD_PORT &= ~LCD_DC_PIN; /* DC pin to low */
  }
  _NOP();  
  i=8;
  do
  {
    if ( Data & 0x80 )
      LCD_PORT |= LCD_IN_PIN; /* IN pin to hight */
    else
      LCD_PORT &= ~LCD_IN_PIN; /* IN pin to low */
    _NOP();
    Data = Data<<1; /* Some additional delay */
    LCD_PORT |= LCD_CLK_PIN; /* CLK pin to hight */
    _NOP();
    i--;
    LCD_PORT &= ~LCD_CLK_PIN; /* CLK pin to low */
  }while(i);
  LCD_PORT |= LCD_CE_PIN; /* CE pin to HI */
}
